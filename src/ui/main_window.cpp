...
void MainWindow::create_tooltips() noexcept {
    ...
    info.hwnd = hwnd_;
    info.lpszText = const_cast<wchar_t*>(L"切换主题");
    info.rect = RECT{kWidth - 150, 12, kWidth - 82, 62};
    (void)SendMessageW(tooltip_, TTM_ADDTOOLW, 0, reinterpret_cast<LPARAM>(&info));
    info.lpszText = const_cast<wchar_t*>(L"打开设置");
    info.rect = RECT{kWidth - 76, 12, kWidth - 20, 62};
    (void)SendMessageW(tooltip_, TTM_ADDTOOLW, 0, reinterpret_cast<LPARAM>(&info));
}
