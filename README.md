...
去 [Releases](../../releases) 下载最新的压缩包，**完整解压到一个文件夹**后双击其中的 `OpenSuperWhisper.App.exe`（不要只把 exe 单独拷出去用，它需要和旁边的文件放在一起）。不需要安装 .NET 或任何其他运行环境（自带）。

## 怎么用

1. 启动后程序常驻系统托盘（任务栏右下角，可能在"显示隐藏的图标"里）。
2. **按住右 Ctrl 说话，松开自动把识别结果打字输入到当前光标所在的位置**——ChatGPT、Claude Code、VSCode、Word、浏览器、微信、邮件都可以。
3. 左键单击托盘图标：显示/隐藏主界面（历史记录）。再点一次隐藏，不会退出程序。也可以按 **Ctrl+Alt+H**（默认组合键，可在设置里改）在任何界面下直接显示/隐藏主界面，不用先切到托盘。
4. 右键单击托盘图标：
   - **显示主界面** — 看最近说过的话，或清空历史记录
   - **设置...** — 改快捷键（点一下输入框，按下想用的键即可绑定；显示/隐藏窗口的快捷键额外支持组合键）、改识别语言
   - **重试初始化** — 模型没加载成功时用这个重试
   - **退出** — 真正关闭程序

内置的是 `small`（多语言）识别模型，中英文都能识别，设置里"识别语言"选"自动检测"或直接指定"中文"都可以；换成更小/更大的模型作为可选项会在后续版本里做。
